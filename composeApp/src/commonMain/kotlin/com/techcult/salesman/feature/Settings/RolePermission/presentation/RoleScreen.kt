...
import com.techcult.salesman.core.presentation.components.MyListItem
...
import com.techcult.salesman.feature.Settings.RolePermission.presentation.component.RoleCard
...
fun RolePermissionScreen(viewModel: RoleViewModel = koinViewModel()) {
    ...
    RolePermissionScreenContent(state, onAction = {
...
fun RolePermissionScreenContent(
    ...
    Scaffold(floatingActionButton = {
        if (deviceConfiguration == DeviceConfiguration.MOBILE_PORTRAIT) {
            FloatingActionButton(modifier = Modifier.padding(bottom = 96.dp), onClick = {
        ...
        Surface(
            ...
            Column(modifier = Modifier.fillMaxSize().padding(LocalPadding.current.normal)) {
                ...
                {
                    Tab(selected = state.selectedTab == 0, onClick = {
                        onAction(RolePermissionAction.OnSelectedTabChanged(0))

                    }, text = { Text("Role") })
                    Tab(selected = state.selectedTab == 1, onClick = {
                        onAction(RolePermissionAction.OnSelectedTabChanged(1))
                ...
                when (state.selectedTab) {
                    0 -> {
                        RoleContent(state, onAction = onAction,deviceConfiguration = deviceConfiguration)
                    }
...
@Composable
fun RoleContent(
    state: RolePermissionUiState = RolePermissionUiState(),
    onAction: (RolePermissionAction) -> Unit,
    deviceConfiguration: DeviceConfiguration
) {
    if(deviceConfiguration==DeviceConfiguration.MOBILE_PORTRAIT)
    {
        RoleList(state, onAction, deviceConfiguration)
...
fun RoleList(
    ...
    LazyColumn {
        ...
        items(state.rolePermission.size)
        {
            if (deviceConfiguration == DeviceConfiguration.MOBILE_PORTRAIT)
            {
                RoleCard(state.rolePermission[it], onAction = onAction)
    ...
    LazyColumn {
        groupedPermissions.forEach { (department, permissions) ->
            ...
            items(permissions.size) { index ->
                MyListItem(
                    title = permissions[index].permissionName,
                    subtitle = permissions[index].permissionDescription
                )
            ...
            )
            {
                Column(modifier = Modifier.padding(LocalPadding.current.normal)) {
                    Row(
                        ...
                        Text(
                            state.selectedRole?.roleName.toString(),
                            style = MaterialTheme.typography.titleMedium
                        )
                        IconButton(onClick = {
                            action(RolePermissionAction.OnViewRoleClicked(false, 0))
                    ...
                    Text("Description")
                    Text(
                        state.selectedRole?.roleDescription.toString(),
                        style = MaterialTheme.typography.bodyMedium
                    )
                    Spacer(modifier = Modifier.height(LocalPadding.current.normal))
                    Row(
                    ...
                    state.permissionChecked.forEach { permission ->
                        Surface(
                            ...
                            Row(
                                ...
                                Checkbox(
                                    checked = state.selectedRole?.permissionId.let {
                                        state.selectedRole?.permissionId?.contains(permission.permissionEntity.permissionId)
                                            ?: false
                                    },
                                    onCheckedChange = {
                                    },
